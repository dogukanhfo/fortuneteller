...
class _StaticImageState extends State<StaticImage> {
  ...
  loadTfModel() async {
    await Tflite.loadModel(
      model: "asset/ssd_mobilenet.tflite",
  ...
  loadJson() async {
    String data = await rootBundle.loadString('asset/comment.json');
    Map<String, dynamic> jsonResult = json.decode(data);
    _figures = jsonResult["figures"];
    print(_figures);
  }
  ...
  List figureList() {
    ...
    return _recognitions!
        .map((e) => _figures
            ?.where((element) => element["id"] == e["detectedClass"])
            .first["comment"])
        .toSet()
        .toList();
  }
    ...
    stackChildren.add(Positioned(
      ...
      child: _image == null
          ...
          Column(
              children: [
                Image.file(_image!),
                Text(figureList().join(" ")),
                const SizedBox(
                  height: 30,
                ),
                AddComment(
                  userId: user.uid,
                  comment: figureList().join(" "),
